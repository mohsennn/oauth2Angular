...
export class ApiService {
  ...
  login(loginPayload) {
    console.log('Affichage de loginPayload', loginPayload);
    const headers = {
  ...
  updateUser(user: User) {
    return this.http.put(this.baseUrl + 'user/' + user.id +
    '?access_token=' + JSON.parse(window.sessionStorage.getItem('token')).access_token, user);
  }
